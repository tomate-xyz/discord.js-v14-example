...
readRecursively("./bot/commands", commandFiles, ".js");
readRecursively("./bot/events", eventFiles, ".js");
...
const registerSlashCommands = async (commands, applicationId, guildId) => {
  try {
    ...
    console.log(
      `${successEmoji} Registered slash commands for guild ${guildId}: ${commandData
        .map((c) => c.name)
        .join(", ")}`
    );
  } catch (error) {
    console.error(
      `${errorEmoji} Failed to register slash commands for guild ${guildId}:`,
      error
    );
    throw error;
  }
  ...
  try {
    ...
    for (const file of commandFiles) {
      try {
        ...
        console.error(
          `${errorEmoji} Failed to load command file ${file}:`,
          error
        );
      }
    }
    ...
    console.log(
      `${successEmoji} All commands registered successfully for guild ${guildId}`
    );
  } catch (error) {
...
const getApplicationId = async () => {
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(
        `Failed to fetch application ID: ${response.status} - ${data.message}`
      );
    }
  } catch (error) {
